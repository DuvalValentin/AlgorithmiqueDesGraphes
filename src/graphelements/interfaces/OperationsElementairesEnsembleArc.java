...
public interface OperationsElementairesEnsembleArc<S>
{
	...
	boolean existeArc(Sommet<S> arrivee, Sommet<S> depart);// Pareil mais avec
																													// deux sommets en
																													// entrée
	boolean existeBoucle();// Rend vrai si il existe un arc tel que
													// depart==arrivee
	boolean existeBoucle(Sommet<S> sommet);// Rend vrai si il existe un arc
																					// (sommet,sommet)
	// Ajout et suppression d'arcs
	// L'arc peut être ajouté seulement si il n'existe pas déja, l'arc est
	// supprimé seulement si il existe
