...
public class ExempleUtilisation
{
	...
	public static void main(String[] args)
	{
		...
		G.existeBoucle();// Savoir si il existe une boucle dans le graphe(ATTENTION
											// ici on ne tient pas compte de la fermeture transitive)
		G.existeBoucle(s3);// Savoir si le existe une boucle au niveau d'un sommet
												// (ATTENTION ici on ne tient pas compte de la fermeture
												// transitive)
		// Appels aux algorithmes
		Parcours.dfs(G);// Juste un parcours du graphe (ne fait absolument rien)
		Parcours.wfs(G,s1);// Pareil
		// logger.info("Le graphe composé de G (arcs en deux temps) :
		// "+FermetureTransitive.Composition(G, G));//En commentaire tant que
		// composition est privé
